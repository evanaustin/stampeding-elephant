...
const enabledBlocks = [ 'core/columns' ];
 ...
 */
 function getParentBgColor(parentBlocks, index) {
	...
	if (index > 0 && directParent && directParent.attributes.bgColor == 'transparent') {
...
const withBackground = createHigherOrderComponent((BlockEdit) => {
	return (props) => {
		...
		const { parentBgColor, bgColor, mediaId } = props.attributes;
		...
		// get parent bg color, if parent exists
		// if (!parentBgColor) {
		...
		// has background color, so percolate children
		if (bgColor) {
			...
			children.forEach((child) => {
				dispatch('core/block-editor').updateBlockAttributes(child.clientId, { parentColor: bgColor });
			}); */

			// props.classes = `has-bgColor-${bgColor} ${getTextColor(bgColor)}`;
		...
		if (mediaId) {
			...
			const children = select('core/block-editor').getBlocksByClientId(props.clientId)[0].innerBlocks;
			children.forEach((child) => {
				dispatch('core/block-editor').updateBlockAttributes(child.clientId, { parentColor: 'image' });
			});

			props.attributes.className = `has-bgImage-${bgImage} post-${mediaId} `;
			props.classes = `has-bgImage-${bgImage} ${getTextColor(bgColor)}`;
